...
Code Cave是指在程式記憶體中一連串的空位（null bytes）。這些空位通常是指程式碼中可以插入自訂指令的區域。例如，如果某個程式碼區塊的記憶體空間允許佔用 5 個位元組，但實際只使用了 3 個位元組，那麼剩下的 2 個位元組就可以用來加入外部程式碼。

# 說明

-f / --file : 有效的 PE 檔案位置（例如：/path/to/calc.exe）。

-p / --payload : Shellcode 負載（例如："\x01\x02\x03...\x0a"）。

-x / --encrypt : 加密主要區段（入口點區段）。

-k / --encryption-key : 定義自訂加密金鑰（僅限 1 個位元組）。

-c / --cave-opcodes : 定義要搜尋的程式碼操作碼列表。

-s / --cave-min-size : 要被視為程式碼洞的最小區域大小。

-e / --egg : 定義自訂的 egg 名稱（ESP 恢復機制）。
...
Links:https://github.com/DarkCoderSc/pe-code-cave-helper/tree/main
